...
First, we load the CSV file, remove all the datapoints where we have an altitudes of '0', filter only to this years data.
Then we display a summary of the data.
...
```
In `r whichyear` I was [somewhere close to Huttwil](http://www.openstreetmap.org/#map=12/`r mean(thisyear$lat)`/`r mean(thisyear$lon)`) in the mean, and [somewhere in the Emmental](http://www.openstreetmap.org/#map=12/`r median(thisyear$lat)`/`r median(thisyear$lon)`) as the median. I've lived at `r mean(thisyear$alt, na.rm=TRUE)` m [AMSL](https://en.wikipedia.org/wiki/Sea_level) in the mean.
...
The northmost point in `r whichyear` is close to [Ingelheim](http://www.openstreetmap.org/#map=12/`r NLocation[c(2)]`/`r NLocation[c(1)]`), which is true.
I spent two days in Ingelheim in January, at the [International Masters competition in Swimming](http://www.ssv-ingelheim.de/veranstaltungen/masterwettkampf/).
...
The most eastern point was in the [Bregenzer Wald](http://www.openstreetmap.org/#map=12/`r ELocation[c(2)]`/`r ELocation[c(1)]`), the first vacation with our (then) newborn daughter.
The most southern point was in [Cervinia](http://www.openstreetmap.org/#map=12/`r SLocation[c(2)]`/`r SLocation[c(1)]`), while skiing in Zermatt.
The most western point was in [Lausanne](http://www.openstreetmap.org/#map=12/`r WLocation[c(2)]`/`r WLocation[c(1)]`), for working on the [GlobalDiagnostiX](http://globaldiagnostix.org/) project.
...
The highest in `r whichyear` was on the [Furgsattel](http://www.openstreetmap.org/#map=12/`r HighestLocation[c(2)]`/`r HighestLocation[c(1)]`), at `r round(max(thisyear$alt, na.rm=TRUE),1)` m AMSL while skiing in Zermat.
The lowest point at `r round(min(thisyear$alt, na.rm=TRUE),1)` m AMSL was at [Germany, also in Ingelheim](http://www.openstreetmap.org/#map=12/`r LowestLocation[c(2)]`/`r LowestLocation[c(1)]`).
